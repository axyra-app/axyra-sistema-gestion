 ...
 * Maneja archivos, documentos, versiones, permisos y colaboración
 */

class AxyraFileDocumentManagementSystem {
  constructor() {
    this.files = [];
    this.documents = [];
    this.versions = [];
  ...
  init() {
    console.log('📁 Inicializando sistema de gestión de archivos y documentos...');
    this.loadFiles();
    this.loadDocuments();
    this.loadVersions();
  ...
  loadDocuments() {
    try {
      const stored = localStorage.getItem('axyra_documents');
      if (stored) {
  ...
  saveDocuments() {
    try {
      localStorage.setItem('axyra_documents', JSON.stringify(this.documents));
    } catch (error) {
  ...
  loadCollaborations() {
    try {
      const stored = localStorage.getItem('axyra_file_collaborations');
      if (stored) {
        this.collaborations = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando colaboraciones:', error);
    }
  }

  saveCollaborations() {
    try {
      localStorage.setItem('axyra_file_collaborations', JSON.stringify(this.collaborations));
    } catch (error) {
      console.error('Error guardando colaboraciones:', error);
    }
  }

  loadTags() {
    try {
      const stored = localStorage.getItem('axyra_file_tags');
      if (stored) {
  ...
  saveTags() {
    try {
      localStorage.setItem('axyra_file_tags', JSON.stringify(this.tags));
    } catch (error) {
  ...
  loadCategories() {
    try {
      const stored = localStorage.getItem('axyra_file_categories');
      if (stored) {
  ...
  loadFolders() {
    try {
      const stored = localStorage.getItem('axyra_file_folders');
      if (stored) {
  ...
  saveFolders() {
    try {
      localStorage.setItem('axyra_file_folders', JSON.stringify(this.folders));
    } catch (error) {
  ...
  loadFileLogs() {
    try {
      const stored = localStorage.getItem('axyra_file_logs');
      if (stored) {
        this.fileLogs = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando logs de archivos:', error);
    }
  }

  saveFileLogs() {
    try {
      localStorage.setItem('axyra_file_logs', JSON.stringify(this.fileLogs));
    } catch (error) {
      console.error('Error guardando logs de archivos:', error);
    }
  }
  ...
  createFile(fileData) {
    const file = {
      ...
      categoryId: fileData.categoryId || 'documents',
      tags: fileData.tags || [],
  ...
  createDocument(documentData) {
    const document = {
      ...
      type: documentData.type || 'text', // text, html, markdown, pdf
      status: documentData.status || 'draft', // draft, review, approved, published
      version: documentData.version || 1,
      isActive: documentData.isActive !== false,
      metadata: {
        createdAt: new Date().toISOString(),
        createdBy: this.getCurrentUser(),
        updatedAt: new Date().toISOString(),
  ...
  createVersion(versionData) {
    ...
    console.log('✅ Versión creada:', version.version);
    return version;
  }
  ...
  createPermission(permissionData) {
    ...
    console.log('✅ Permiso creado:', permission.role);
    return permission;
  }
  ...
  createCollaboration(collaborationData) {
    const collaboration = {
      ...
      fileId: collaborationData.fileId,
      userId: collaborationData.userId,
      action: collaborationData.action, // comment, edit, review, approve
    ...
    console.log('✅ Colaboración creada:', collaboration.action);
  ...
  createFileLog(logData) {
    const log = {
      ...
      category: logData.category || 'general', // general, upload, download, delete, share
      data: logData.data || {},
    ...
    console.log('✅ Log de archivo creado:', log.message);
    return log;
  }
  ...
  showFileManagementDashboard() {
    const dashboard = document.createElement('div');
    dashboard.id = 'file-management-dashboard';
    dashboard.innerHTML = `
      <div class="file-management-dashboard-overlay">
        <div class="file-management-dashboard-container">
          <div class="file-management-dashboard-header">
            <h3>📁 Dashboard de Gestión de Archivos</h3>
            <div class="file-management-dashboard-actions">
              <button class="btn btn-primary" onclick="axyraFileDocumentManagementSystem.showCreateFileDialog()">Nuevo Archivo</button>
              <button class="btn btn-secondary" onclick="axyraFileDocumentManagementSystem.showCreateDocumentDialog()">Nuevo Documento</button>
              <button class="btn btn-close" onclick="document.getElementById('file-management-dashboard').remove()">×</button>
            </div>
          </div>
          <div class="file-management-dashboard-body">
            <div class="file-management-dashboard-stats">
              ${this.renderFileStats()}
            </div>
            <div class="file-management-dashboard-content">
              <div class="file-management-dashboard-tabs">
                ...
                <button class="tab-btn" data-tab="versions">Versiones</button>
                <button class="tab-btn" data-tab="permissions">Permisos</button>
                <button class="tab-btn" data-tab="collaborations">Colaboraciones</button>
                <button class="tab-btn" data-tab="tags">Etiquetas</button>
                <button class="tab-btn" data-tab="categories">Categorías</button>
                <button class="tab-btn" data-tab="folders">Carpetas</button>
                <button class="tab-btn" data-tab="logs">Logs</button>
              </div>
              <div class="file-management-dashboard-tab-content">
                ...
                <div class="tab-content" id="collaborations-tab">
                  ${this.renderCollaborationsList()}
                </div>
    ...
    return `
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-label">Colaboraciones Activas</div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
          <div class="stat-value">${stats.activeFolders}</div>
          <div class="stat-label">Carpetas Activas</div>
        </div>
        <div class="stat-card">
  ...
  renderOverview() {
    ...
    return `
      <div class="overview-grid">
        <div class="overview-card">
          ...
          <div class="file-status">
            <div class="status-item">
              ...
              <span>${stats.activeFiles}/${stats.totalFiles}</span>
            </div>
            <div class="status-item">
              <span>Documentos Activos</span>
              <span>${stats.activeDocuments}/${stats.totalDocuments}</span>
            </div>
            <div class="status-item">
        ...
        <div class="overview-card">
          <h4>Permisos y Colaboración</h4>
          <div class="permissions-collaboration">
            <div class="permission-item">
              <span>Permisos Activos</span>
              <span>${stats.activePermissions}</span>
            </div>
            <div class="permission-item">
              <span>Colaboraciones Activas</span>
        ...
        <div class="overview-card">
          ...
          <div class="organization">
            ...
            <div class="org-item">
              ...
              <span>${stats.activeCategories}</span>
            </div>
            <div class="org-item">
              <span>Carpetas Activas</span>
              <span>${stats.activeFolders}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    return files
      ...
      <div class="file-card">
        ...
        <div class="file-info">
          ...
          <p>Público: ${file.isPublic ? 'Sí' : 'No'}</p>
        </div>
        <div class="file-actions">
          <button onclick="axyraFileDocumentManagementSystem.showFileDetails('${file.id}')">Ver</button>
    ...
    return documents
      ...
      <div class="document-card">
        ...
        <div class="document-info">
          ...
          <p>Contenido: ${document.content.substring(0, 100)}...</p>
        </div>
        <div class="document-actions">
          <button onclick="axyraFileDocumentManagementSystem.showDocumentDetails('${document.id}')">Ver</button>
    ...
    return versions
      ...
      <div class="version-card">
        ...
        <div class="version-info">
          ...
          <p>Cambios: ${version.changes}</p>
          <p>Tamaño: ${this.formatFileSize(version.size)}</p>
        </div>
        <div class="version-actions">
          <button onclick="axyraFileDocumentManagementSystem.showVersionDetails('${version.id}')">Ver</button>
    ...
    return permissions
      ...
      <div class="permission-card">
        ...
        <div class="permission-info">
          ...
          <p>Usuario: ${permission.userId}</p>
          <p>Lectura: ${permission.canRead ? 'Sí' : 'No'}</p>
          <p>Escritura: ${permission.canWrite ? 'Sí' : 'No'}</p>
          <p>Eliminación: ${permission.canDelete ? 'Sí' : 'No'}</p>
          <p>Compartir: ${permission.canShare ? 'Sí' : 'No'}</p>
        </div>
        <div class="permission-actions">
          <button onclick="axyraFileDocumentManagementSystem.showPermissionDetails('${permission.id}')">Ver</button>
    ...
    return collaborations
      ...
      <div class="collaboration-card">
        ...
        <div class="collaboration-info">
          ...
          <p>Usuario: ${collaboration.userId}</p>
  ...
  renderTagsList() {
    ...
    return tags
      ...
      <div class="tag-card">
        ...
        <div class="tag-info">
          <p>${tag.description}</p>
          <p>Estado: ${tag.isActive ? 'Activo' : 'Inactivo'}</p>
        </div>
        <div class="tag-actions">
          <button onclick="axyraFileDocumentManagementSystem.showTagDetails('${tag.id}')">Ver</button>
    ...
    return categories
      ...
      <div class="category-card">
        ...
        <div class="category-actions">
          <button onclick="axyraFileDocumentManagementSystem.showCategoryDetails('${category.id}')">Ver</button>
    ...
    return folders
      ...
      <div class="folder-card">
        ...
        <div class="folder-info">
          ...
          <p>Padre: ${folder.parentId || 'Raíz'}</p>
        </div>
        <div class="folder-actions">
          <button onclick="axyraFileDocumentManagementSystem.showFolderDetails('${folder.id}')">Ver</button>
  ...
  showCreateFileDialog() {
    ...
    if (name) {
      const type = prompt('Tipo de archivo:');
      const size = prompt('Tamaño en bytes:');
      this.createFile({ name, type, size: parseInt(size) });
    }
  }
  ...
  showCreateDocumentDialog() {
    ...
    if (title) {
      ...
      const type = prompt('Tipo (text, html, markdown, pdf):');
      this.createDocument({ title, content, type });
    }
  }
  ...
  editCollaboration(collaborationId) {
    ...
    if (collaboration) {
      ...
      if (newContent !== null) {
        collaboration.content = newContent;
        this.saveCollaborations();
      }
    }
  }
...
// Inicializar sistema de gestión de archivos y documentos
let axyraFileDocumentManagementSystem;
document.addEventListener('DOMContentLoaded', () => {
  axyraFileDocumentManagementSystem = new AxyraFileDocumentManagementSystem();
  window.axyraFileDocumentManagementSystem = axyraFileDocumentManagementSystem;
});
