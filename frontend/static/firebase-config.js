/**
 * 🔥 FIREBASE CONFIG - AXYRA SISTEMA DE GESTIÓN
...
// Si no se pudo inicializar, intentar cada 500ms por 10 segundos
if (!firebaseInitialized) {
    let attempts = 0;
    ...
    const checkFirebase = setInterval(() => {
        attempts++;
        if (checkFirebaseAvailability() || attempts >= maxAttempts) {
            clearInterval(checkFirebase);
            if (!firebaseInitialized) {
                console.warn('⚠️ Firebase no se pudo inicializar, continuando en modo offline');
            }
        }
    }, 500);
}
...
async function saveFirestoreData(collection, docId, data, options = {}) {
    if (isFirebaseAvailable()) {
        try {
            ...
            const docRef = window.axyraFirebase.firestore
