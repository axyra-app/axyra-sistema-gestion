...
class AxyraTaskManagement {
  ...
  getUpcomingTasks(days = 7) {
    ...
    return this.tasks.filter(t => 
      t.dueDate && t.dueDate <= futureDateStr && t.status !== 'Completada'
    );
  }
  ...
  notifyTaskDeadline(task, daysUntilDue) {
    if (window.axyraNotificationSystem) {
      const message = daysUntilDue === 0 ? 
        ...
        `Tarea vence en ${daysUntilDue} días: ${task.title}`;
  ...
  convertTasksToCSV() {
    ...
    // Encabezados
    rows.push(['ID', 'Título', 'Descripción', 'Categoría', 'Prioridad', 'Estado', 'Asignado', 'Proyecto', 'Fecha Vencimiento', 'Progreso', 'Creado']);
