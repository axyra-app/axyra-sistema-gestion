/**
 * AXYRA - Sistema de Pagos
 * Integración con Bancolombia y Nequi para pagos mensuales
 */
...
class AxyraPaymentSystem {
  ...
  loadSubscriptions() {
    try {
      const stored = localStorage.getItem('axyra_subscriptions');
      if (stored) {
        this.subscriptions = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando suscripciones:', error);
    }
  }
  ...
  loadPaymentMethods() {
    try {
      const stored = localStorage.getItem('axyra_payment_methods');
      if (stored) {
  ...
  savePaymentMethods() {
    try {
      localStorage.setItem('axyra_payment_methods', JSON.stringify(this.paymentMethods));
    } catch (error) {
  ...
  loadPaymentHistory() {
    try {
      const stored = localStorage.getItem('axyra_payment_history');
      if (stored) {
        this.paymentHistory = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando historial de pagos:', error);
    }
  }
  ...
  savePaymentHistory() {
    try {
      ...
      console.error('Error guardando historial de pagos:', error);
    }
  }

  loadPlans() {
    try {
      const stored = localStorage.getItem('axyra_plans');
      if (stored) {
        this.plans = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando planes:', error);
    }
  }

  savePlans() {
    try {
      localStorage.setItem('axyra_plans', JSON.stringify(this.plans));
    } catch (error) {
  ...
  createSubscription(subscriptionData) {
    const subscription = {
      ...
      userId: subscriptionData.userId || this.getCurrentUser(),
      planId: subscriptionData.planId,
      status: subscriptionData.status || 'active', // active, cancelled, expired, pending
      startDate: subscriptionData.startDate || new Date().toISOString(),
      endDate: subscriptionData.endDate || this.calculateEndDate(subscriptionData.planId),
      paymentMethod: subscriptionData.paymentMethod || 'bancolombia',
  ...
  createPaymentMethod(methodData) {
    const method = {
      ...
      userId: methodData.userId || this.getCurrentUser(),
      type: methodData.type, // bancolombia, nequi
    ...
    console.log('✅ Método de pago creado:', method.type);
    return method;
  }
  ...
  processPayment(paymentData) {
    ...
    if (paymentResult.success) {
      ...
      return { success: true, payment, message: 'Pago procesado exitosamente' };
    } else {
      console.error('❌ Error procesando pago:', paymentResult.error);
      ...
      <div class="payment-dashboard-overlay">
        <div class="payment-dashboard-container">
          <div class="payment-dashboard-header">
            ...
            <div class="payment-dashboard-actions">
              <button class="btn btn-primary" onclick="axyraPaymentSystem.showPlansDialog()">Ver Planes</button>
              <button class="btn btn-secondary" onclick="axyraPaymentSystem.showPaymentMethodsDialog()">Métodos de Pago</button>
          ...
          <div class="payment-dashboard-body">
            <div class="payment-dashboard-stats">
              ${this.renderPaymentStats()}
            </div>
            <div class="payment-dashboard-content">
              <div class="payment-dashboard-tabs">
                <button class="tab-btn active" data-tab="overview">Resumen</button>
                <button class="tab-btn" data-tab="plans">Planes</button>
                <button class="tab-btn" data-tab="subscriptions">Suscripciones</button>
                <button class="tab-btn" data-tab="payments">Pagos</button>
                <button class="tab-btn" data-tab="methods">Métodos</button>
              </div>
              <div class="payment-dashboard-tab-content">
    ...
    return `
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-label">Ingresos Totales (COP)</div>
        </div>
      </div>
    `;
  }
        ...
        <div class="overview-card">
          ...
          <div class="subscription-status">
            <div class="status-item">
              ...
              <span>${stats.activeSubscriptions}/${stats.totalSubscriptions}</span>
            </div>
            <div class="status-item">
        ...
        <div class="overview-card">
          ...
          <div class="revenue-info">
            <div class="revenue-item">
              ...
              <span>$${stats.totalRevenue.toLocaleString()} COP</span>
            </div>
            <div class="revenue-item">
              <span>Pagos Exitosos</span>
              <span>${stats.successfulPayments}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    return this.plans.map(plan => `
      <div class="plan-card">
        <div class="plan-header">
          <h5>${plan.name}</h5>
          <span class="plan-price">$${plan.price.toLocaleString()} COP/mes</span>
        </div>
        <div class="plan-info">
    ...
    return subscriptions.map(subscription => `
      <div class="subscription-card">
        ...
        <div class="subscription-info">
          ...
          <p>Fin: ${new Date(subscription.endDate).toLocaleDateString()}</p>
        </div>
        <div class="subscription-actions">
          <button onclick="axyraPaymentSystem.showSubscriptionDetails('${subscription.id}')">Ver</button>
    ...
    return payments.map(payment => `
      <div class="payment-card">
        ...
        <div class="payment-info">
          <p>Monto: $${payment.amount.toLocaleString()} COP</p>
          <p>Método: ${payment.method}</p>
          <p>Fecha: ${new Date(payment.timestamp).toLocaleString()}</p>
    ...
    return methods.map(method => `
      <div class="method-card">
        ...
        <div class="method-info">
          ...
          <p>Nombre: ${method.accountName}</p>
          <p>Por defecto: ${method.isDefault ? 'Sí' : 'No'}</p>
        </div>
        <div class="method-actions">
          <button onclick="axyraPaymentSystem.showMethodDetails('${method.id}')">Ver</button>
  ...
  showPaymentMethodsDialog() {
    const type = prompt('Tipo de método (bancolombia, nequi):');
    if (type) {
      const accountNumber = prompt('Número de cuenta:');
      const accountName = prompt('Nombre del titular:');
      this.createPaymentMethod({ type, accountNumber, accountName });
    }
  }
  ...
  editMethod(methodId) {
    ...
    if (method) {
      ...
      if (newAccountNumber) {
        method.accountNumber = newAccountNumber;
        this.savePaymentMethods();
      }
    }
  }
...
let axyraPaymentSystem;
document.addEventListener('DOMContentLoaded', () => {
  axyraPaymentSystem = new AxyraPaymentSystem();
  window.axyraPaymentSystem = axyraPaymentSystem;
});
