/**
 * AXYRA - Sistema de Gestión de Ventas y E-commerce
 * Maneja productos, catálogo, carrito, pedidos, pagos y clientes
 */

class AxyraSalesEcommerceSystem {
  constructor() {
    this.products = [];
    this.categories = [];
    this.customers = [];
  ...
  init() {
    console.log('🛒 Inicializando sistema de ventas y e-commerce...');
    this.loadProducts();
    this.loadCategories();
    this.loadCustomers();
    this.loadOrders();
  ...
  loadProducts() {
    try {
      const stored = localStorage.getItem('axyra_sales_products');
      if (stored) {
  ...
  saveProducts() {
    try {
      localStorage.setItem('axyra_sales_products', JSON.stringify(this.products));
    } catch (error) {
  ...
  loadCategories() {
    try {
      const stored = localStorage.getItem('axyra_sales_categories');
      if (stored) {
  ...
  saveCategories() {
    try {
      localStorage.setItem('axyra_sales_categories', JSON.stringify(this.categories));
    } catch (error) {
  ...
  loadCustomers() {
    try {
      const stored = localStorage.getItem('axyra_sales_customers');
      if (stored) {
  ...
  saveCustomers() {
    try {
      localStorage.setItem('axyra_sales_customers', JSON.stringify(this.customers));
    } catch (error) {
  ...
  loadOrders() {
    try {
      const stored = localStorage.getItem('axyra_sales_orders');
      if (stored) {
  ...
  saveOrders() {
    try {
      localStorage.setItem('axyra_sales_orders', JSON.stringify(this.orders));
    } catch (error) {
  ...
  loadOrderItems() {
    try {
      const stored = localStorage.getItem('axyra_sales_order_items');
      if (stored) {
        this.orderItems = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando items de pedidos:', error);
    }
  }

  saveOrderItems() {
    try {
      localStorage.setItem('axyra_sales_order_items', JSON.stringify(this.orderItems));
    } catch (error) {
      console.error('Error guardando items de pedidos:', error);
    }
  }

  loadPayments() {
    try {
      const stored = localStorage.getItem('axyra_sales_payments');
      if (stored) {
  ...
  savePayments() {
    try {
      localStorage.setItem('axyra_sales_payments', JSON.stringify(this.payments));
    } catch (error) {
  ...
  loadShipping() {
    try {
      const stored = localStorage.getItem('axyra_sales_shipping');
      if (stored) {
        this.shipping = JSON.parse(stored);
      }
    } catch (error) {
  ...
  saveShipping() {
    try {
      localStorage.setItem('axyra_sales_shipping', JSON.stringify(this.shipping));
    } catch (error) {
  ...
  loadCoupons() {
    try {
      const stored = localStorage.getItem('axyra_sales_coupons');
      if (stored) {
        this.coupons = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando cupones:', error);
    }
  }

  saveCoupons() {
    try {
      localStorage.setItem('axyra_sales_coupons', JSON.stringify(this.coupons));
    } catch (error) {
  ...
  loadReviews() {
    try {
      const stored = localStorage.getItem('axyra_sales_reviews');
      if (stored) {
  ...
  saveReviews() {
    try {
      localStorage.setItem('axyra_sales_reviews', JSON.stringify(this.reviews));
    } catch (error) {
  ...
  loadWishlists() {
    try {
      const stored = localStorage.getItem('axyra_sales_wishlists');
      if (stored) {
        this.wishlists = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando listas de deseos:', error);
    }
  }

  saveWishlists() {
    try {
      localStorage.setItem('axyra_sales_wishlists', JSON.stringify(this.wishlists));
    } catch (error) {
      console.error('Error guardando listas de deseos:', error);
    }
  }

  loadCart() {
    try {
      const stored = localStorage.getItem('axyra_sales_cart');
      if (stored) {
  ...
  saveCart() {
    try {
      localStorage.setItem('axyra_sales_cart', JSON.stringify(this.cart));
    } catch (error) {
  ...
  setupEventListeners() {
    ...
    document.addEventListener('orderChanged', (event) => {
      this.handleOrderChange(event.detail);
    });
  }
  ...
  createOrder(orderData) {
    const order = {
      ...
      shippingStatus: orderData.shippingStatus || 'pending', // pending, shipped, delivered, returned
  ...
  createOrderItem(orderItemData) {
    ...
    console.log('✅ Item de pedido creado:', orderItem.id);
    return orderItem;
  }
  ...
  createPayment(paymentData) {
    const payment = {
      ...
      method: paymentData.method, // credit_card, debit_card, bank_transfer, cash, digital_wallet
  ...
  createCoupon(couponData) {
    const coupon = {
      ...
      type: couponData.type || 'percentage', // percentage, fixed_amount, free_shipping
  ...
  updateCartQuantity(productId, quantity) {
    ...
    console.log('✅ Cantidad actualizada en carrito:', productId, quantity);
    return this.cart;
  }
  ...
  generateOrderNumber() {
    ...
    return `ORD-${year}${month}-${String(count).padStart(6, '0')}`;
  }
  ...
  showSalesDashboard() {
    ...
    dashboard.innerHTML = `
      <div class="sales-dashboard-overlay">
        <div class="sales-dashboard-container">
          <div class="sales-dashboard-header">
            ...
            <div class="sales-dashboard-actions">
              <button class="btn btn-primary" onclick="axyraSalesEcommerceSystem.showCreateProductDialog()">Nuevo Producto</button>
              <button class="btn btn-secondary" onclick="axyraSalesEcommerceSystem.showCreateOrderDialog()">Nuevo Pedido</button>
              <button class="btn btn-close" onclick="document.getElementById('sales-dashboard').remove()">×</button>
            </div>
          </div>
          <div class="sales-dashboard-body">
            <div class="sales-dashboard-stats">
              ${this.renderSalesStats()}
            </div>
            <div class="sales-dashboard-content">
              <div class="sales-dashboard-tabs">
                ...
                <button class="tab-btn" data-tab="cart">Carrito</button>
              </div>
              <div class="sales-dashboard-tab-content">
    ...
    return products.map(product => `
      <div class="product-card">
        ...
        <div class="product-actions">
          <button onclick="axyraSalesEcommerceSystem.addToCart('${product.id}')">Agregar al Carrito</button>
  ...
  renderCart() {
    if (this.cart.length === 0) {
      return '<div class="empty-cart">El carrito está vacío</div>';
    }
    ...
    return `
      ...
      <div class="cart-total">
        ...
        <button onclick="axyraSalesEcommerceSystem.checkout()">Proceder al Pago</button>
      </div>
    `;
  }
  ...
  showCreateProductDialog() {
    ...
    if (name) {
      ...
      this.createProduct({ name, description, price, categoryId });
    }
  }
  ...
  showCreateOrderDialog() {
    ...
    if (customerId) {
      const total = parseFloat(prompt('Total del pedido:'));
      this.createOrder({ customerId, total });
    }
  }
...
let axyraSalesEcommerceSystem;
document.addEventListener('DOMContentLoaded', () => {
  axyraSalesEcommerceSystem = new AxyraSalesEcommerceSystem();
