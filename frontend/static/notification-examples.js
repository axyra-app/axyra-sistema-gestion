...
class AxyraNotificationExamples {
  ...
  showExampleNotification(index = 0) {
    if (window.axyraNotificationSystem) {
      ...
      window.axyraNotificationSystem.showNotification(example.message, example.type, example.duration, example.action);
    }
  }
  ...
  showPaymentSuccess() {
    if (window.axyraNotificationSystem) {
      window.axyraNotificationSystem.success('Tu suscripción al Plan Profesional ha sido activada', 5000, null);
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // NO mostrar notificación automáticamente
  // setTimeout(() => {
