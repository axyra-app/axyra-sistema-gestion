/**
 * AXYRA - Sistema de Gestión de Usuarios Avanzada
 * Maneja usuarios, roles, permisos, autenticación y autorización
 */

class AxyraAdvancedUserManagementSystem {
  constructor() {
    this.users = [];
    this.roles = [];
    this.permissions = [];
  ...
  init() {
    console.log('👥 Inicializando sistema de gestión de usuarios avanzada...');
    this.loadUsers();
    this.loadRoles();
  ...
  loadRoles() {
    try {
      const stored = localStorage.getItem('axyra_roles');
      if (stored) {
  ...
  loadPermissions() {
    try {
      const stored = localStorage.getItem('axyra_permissions');
      if (stored) {
  ...
  loadUserRoles() {
    try {
      const stored = localStorage.getItem('axyra_user_roles');
      if (stored) {
        this.userRoles = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando roles de usuario:', error);
    }
  }

  saveUserRoles() {
    try {
      localStorage.setItem('axyra_user_roles', JSON.stringify(this.userRoles));
    } catch (error) {
  ...
  loadRolePermissions() {
    try {
      const stored = localStorage.getItem('axyra_role_permissions');
      if (stored) {
        this.rolePermissions = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando permisos de rol:', error);
    }
  }

  saveRolePermissions() {
    try {
      localStorage.setItem('axyra_role_permissions', JSON.stringify(this.rolePermissions));
    } catch (error) {
  ...
  loadUserSessions() {
    try {
      const stored = localStorage.getItem('axyra_user_sessions');
      if (stored) {
        this.userSessions = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando sesiones de usuario:', error);
    }
  }

  saveUserSessions() {
    try {
      localStorage.setItem('axyra_user_sessions', JSON.stringify(this.userSessions));
    } catch (error) {
      console.error('Error guardando sesiones de usuario:', error);
    }
  }

  loadUserActivities() {
    try {
      const stored = localStorage.getItem('axyra_user_activities');
      if (stored) {
        this.userActivities = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando actividades de usuario:', error);
    }
  }

  saveUserActivities() {
    try {
      localStorage.setItem('axyra_user_activities', JSON.stringify(this.userActivities));
    } catch (error) {
      console.error('Error guardando actividades de usuario:', error);
    }
  }

  loadUserPreferences() {
    try {
      const stored = localStorage.getItem('axyra_user_preferences');
      if (stored) {
  ...
  loadUserGroups() {
    try {
      const stored = localStorage.getItem('axyra_user_groups');
      if (stored) {
        this.userGroups = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando grupos de usuario:', error);
    }
  }

  saveUserGroups() {
    try {
      localStorage.setItem('axyra_user_groups', JSON.stringify(this.userGroups));
    } catch (error) {
      console.error('Error guardando grupos de usuario:', error);
    }
  }

  loadUserLogs() {
    try {
      const stored = localStorage.getItem('axyra_user_logs');
      if (stored) {
        this.userLogs = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando logs de usuario:', error);
    }
  }

  saveUserLogs() {
    try {
      localStorage.setItem('axyra_user_logs', JSON.stringify(this.userLogs));
    } catch (error) {
      console.error('Error guardando logs de usuario:', error);
    }
  }
  ...
  createUserRole(userRoleData) {
    const userRole = {
      ...
      assignedAt: userRoleData.assignedAt || new Date().toISOString(),
      assignedBy: userRoleData.assignedBy || this.getCurrentUser(),
    ...
    console.log('✅ Rol de usuario creado:', userRole.roleId);
    return userRole;
  }

  createRolePermission(rolePermissionData) {
    const rolePermission = {
      id: 'role_permission_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9),
      roleId: rolePermissionData.roleId,
      permissionId: rolePermissionData.permissionId,
      grantedAt: rolePermissionData.grantedAt || new Date().toISOString(),
      grantedBy: rolePermissionData.grantedBy || this.getCurrentUser(),
    ...
    console.log('✅ Permiso de rol creado:', rolePermission.permissionId);
  ...
  createUserSession(sessionData) {
    ...
    console.log('✅ Sesión de usuario creada:', session.id);
    return session;
  }
  ...
  createUserActivity(activityData) {
    const activity = {
      ...
      action: activityData.action, // login, logout, create, update, delete, view
    ...
    console.log('✅ Actividad de usuario creada:', activity.action);
    return activity;
  }

  createUserPreference(preferenceData) {
    const preference = {
      ...
      category: preferenceData.category || 'general',
      isActive: preferenceData.isActive !== false,
      metadata: {
        createdAt: new Date().toISOString(),
        createdBy: this.getCurrentUser(),
        updatedAt: new Date().toISOString(),
    ...
    console.log('✅ Preferencia de usuario creada:', preference.key);
    return preference;
  }
  ...
  createUserGroup(groupData) {
    ...
    console.log('✅ Grupo de usuario creado:', group.name);
    return group;
  }

  createUserLog(logData) {
    const log = {
      ...
      category: logData.category || 'general', // general, authentication, authorization, activity
      data: logData.data || {},
      timestamp: new Date().toISOString(),
      metadata: {
        createdBy: this.getCurrentUser()
    ...
    console.log('✅ Log de usuario creado:', log.message);
    return log;
  }
  ...
  showUserManagementDashboard() {
    ...
    dashboard.id = 'user-management-dashboard';
    dashboard.innerHTML = `
      <div class="user-management-dashboard-overlay">
        <div class="user-management-dashboard-container">
          <div class="user-management-dashboard-header">
            <h3>👥 Dashboard de Gestión de Usuarios</h3>
            <div class="user-management-dashboard-actions">
              <button class="btn btn-primary" onclick="axyraAdvancedUserManagementSystem.showCreateUserDialog()">Nuevo Usuario</button>
              <button class="btn btn-secondary" onclick="axyraAdvancedUserManagementSystem.showCreateRoleDialog()">Nuevo Rol</button>
              <button class="btn btn-close" onclick="document.getElementById('user-management-dashboard').remove()">×</button>
            </div>
          </div>
          <div class="user-management-dashboard-body">
            <div class="user-management-dashboard-stats">
              ${this.renderUserStats()}
            </div>
            <div class="user-management-dashboard-content">
              <div class="user-management-dashboard-tabs">
              ...
              <div class="user-management-dashboard-tab-content">
  ...
  renderUserStats() {
    ...
    return `
      <div class="stats-grid">
        ...
        <div class="stat-card">
          <div class="stat-value">${stats.totalPreferences}</div>
          <div class="stat-label">Total Preferencias</div>
        </div>
        <div class="stat-card">
        ...
        <div class="overview-card">
          ...
          <div class="roles-permissions">
            <div class="role-item">
              ...
              <span>${stats.activeRoles}</span>
            </div>
            <div class="role-item">
              <span>Permisos Activos</span>
              <span>${stats.activePermissions}</span>
            </div>
            <div class="role-item">
              <span>Asignaciones de Rol</span>
              <span>${stats.activeUserRoles}</span>
            </div>
          </div>
        </div>
        <div class="overview-card">
          <h4>Actividad y Grupos</h4>
          <div class="activity-groups">
            <div class="activity-item">
              <span>Total Actividades</span>
              <span>${stats.totalActivities}</span>
            </div>
            <div class="activity-item">
            ...
            <div class="activity-item">
              <span>Total Preferencias</span>
              <span>${stats.totalPreferences}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    return users.map(user => `
      <div class="user-card">
        ...
        <div class="user-info">
          ...
          <p>Último login: ${user.lastLogin ? new Date(user.lastLogin).toLocaleString() : 'Nunca'}</p>
        </div>
        <div class="user-actions">
          <button onclick="axyraAdvancedUserManagementSystem.showUserDetails('${user.id}')">Ver</button>
    ...
    return roles.map(role => `
      <div class="role-card">
        <div class="role-header">
          <h5>${role.name}</h5>
          <span class="role-level level-${role.level}">Nivel ${role.level}</span>
        </div>
        <div class="role-info">
        ...
        <div class="role-actions">
          <button onclick="axyraAdvancedUserManagementSystem.showRoleDetails('${role.id}')">Ver</button>
    ...
    return permissions.map(permission => `
      <div class="permission-card">
        <div class="permission-header">
          ...
          <span class="permission-category category-${permission.category}">${permission.category}</span>
        </div>
        <div class="permission-info">
          <p>${permission.description}</p>
          <p>Estado: ${permission.isActive ? 'Activo' : 'Inactivo'}</p>
        </div>
        <div class="permission-actions">
          <button onclick="axyraAdvancedUserManagementSystem.showPermissionDetails('${permission.id}')">Ver</button>
    ...
    return sessions.map(session => `
      <div class="session-card">
        ...
        <div class="session-info">
          ...
          <p>Iniciada: ${new Date(session.startedAt).toLocaleString()}</p>
          <p>Última actividad: ${new Date(session.lastActivity).toLocaleString()}</p>
        </div>
        <div class="session-actions">
          <button onclick="axyraAdvancedUserManagementSystem.showSessionDetails('${session.id}')">Ver</button>
    ...
    return activities.map(activity => `
      <div class="activity-card">
        ...
        <div class="activity-info">
          <p>Usuario: ${activity.userId}</p>
          <p>Recurso: ${activity.resource}</p>
    ...
    return preferences.map(preference => `
      <div class="preference-card">
        <div class="preference-header">
          ...
          <span class="preference-category category-${preference.category}">${preference.category}</span>
        </div>
        <div class="preference-info">
          <p>Usuario: ${preference.userId}</p>
          <p>Valor: ${preference.value}</p>
          <p>Estado: ${preference.isActive ? 'Activo' : 'Inactivo'}</p>
        </div>
        <div class="preference-actions">
          <button onclick="axyraAdvancedUserManagementSystem.showPreferenceDetails('${preference.id}')">Ver</button>
    ...
    return groups.map(group => `
      <div class="group-card">
        ...
        <div class="group-actions">
          <button onclick="axyraAdvancedUserManagementSystem.showGroupDetails('${group.id}')">Ver</button>
  ...
  showCreateUserDialog() {
    ...
    if (username) {
      ...
      this.createUser({ username, email, firstName, lastName });
    }
  }
  ...
  showCreateRoleDialog() {
    ...
    if (name) {
      ...
      const level = prompt('Nivel (0-100):');
      this.createRole({ name, description, level: parseInt(level) });
    }
  }
...
// Inicializar sistema de gestión de usuarios avanzada
let axyraAdvancedUserManagementSystem;
document.addEventListener('DOMContentLoaded', () => {
  axyraAdvancedUserManagementSystem = new AxyraAdvancedUserManagementSystem();
