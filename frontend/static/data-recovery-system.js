/**
 * AXYRA - Sistema de Recuperación de Datos
 * Maneja respaldos automáticos y recuperación de datos
 */

class AxyraDataRecoverySystem {
  constructor() {
    this.backupConfig = {
      autoBackup: true,
  ...
  init() {
    console.log('💾 Inicializando sistema de recuperación de datos...');
  ...
  loadBackupConfig() {
    try {
      ...
      if (stored) {
        ...
        this.backupConfig = { ...this.backupConfig, ...parsed };
      }
    } catch (error) {
      console.warn('Error cargando configuración de backup:', error);
    }
  }
  ...
  loadBackups() {
    try {
      const stored = localStorage.getItem('axyra_backups');
      if (stored) {
        this.backups = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando backups:', error);
    }
  }
  ...
  saveBackups() {
    try {
      ...
      console.error('Error guardando backups:', error);
    }
  }
  ...
  setupBeforeUnload() {
    window.addEventListener('beforeunload', (event) => {
      if (this.hasUnsavedChanges()) {
        ...
        event.returnValue = '¿Está seguro de que desea salir? Tiene cambios sin guardar.';
  ...
  setupRecoveryPoints() {
    // Crear puntos de recuperación automáticos
    setInterval(() => {
  ...
  saveRecoveryPoints() {
    try {
      localStorage.setItem('axyra_recovery_points', JSON.stringify(this.recoveryPoints));
    } catch (error) {
      console.error('Error guardando puntos de recuperación:', error);
    }
  }
  ...
  async exportAllData() {
    ...
    const criticalKeys = [
      ...
      'axyra_inventario',
      'axyra_cuadre_caja',
      'axyra_config_empresa',
      'axyra_config_sistema',
      'axyra_usuarios',
      'axyra_roles',
      'axyra_audit_log',
      'axyra_security_log',
    ...
    criticalKeys.forEach((key) => {
      ...
      if (value) {
        try {
          ...
          data[key] = value; // Guardar como string si no es JSON válido
        }
      }
    });
  ...
  async compressData(data) {
    // Simular compresión (en un entorno real usaría una librería como pako)
  ...
  async encryptData(data) {
    // Simular encriptación (en un entorno real usaría Web Crypto API)
    return btoa(JSON.stringify(data));
  }
    ...
    modal.innerHTML = `
      <div class="modal-container">
              ...
              ${this.backups
                ...
                <div class="backup-item">
                  <div class="backup-info">
                    ...
                    <p>${backup.description || 'Sin descripción'}</p>
                    <small>Tamaño: ${this.formatBytes(backup.size)}</small>
                  </div>
                  <div class="backup-actions">
                    <button onclick="window.axyraDataRecoverySystem.restoreBackup('${backup.id}')" 
                            class="btn btn-primary btn-sm">
                      Restaurar
                    </button>
                    <button onclick="window.axyraDataRecoverySystem.downloadBackup('${backup.id}')" 
              ...
              ${this.recoveryPoints
                ...
                <div class="recovery-point-item">
                  <div class="point-info">
                    ...
                    <small>Tamaño: ${this.formatBytes(point.size)}</small>
                  </div>
                  <div class="point-actions">
                    <button onclick="window.axyraDataRecoverySystem.restoreFromPoint('${point.id}')" 
                            class="btn btn-primary btn-sm">
                      Restaurar
                    </button>
                  </div>
        ...
        <div class="modal-footer">
          ...
          <button onclick="window.axyraDataRecoverySystem.createBackup('MANUAL', 'Backup manual')" 
  ...
  async downloadBackup(backupId) {
    ...
    try {
      ...
      link.href = url;
      link.download = `axyra-backup-${backup.id}.json`;
      ...
      reader.onload = async (event) => {
        try {
          ...
          const backup = {
            ...
            timestamp: new Date().toISOString(),
            type: 'UPLOADED',
            description: 'Backup subido desde archivo',
            version: '1.0.0',
            data: data,
            size: file.size,
...
// Inicializar sistema de recuperación de datos
let axyraDataRecoverySystem;
document.addEventListener('DOMContentLoaded', () => {
  axyraDataRecoverySystem = new AxyraDataRecoverySystem();
  window.axyraDataRecoverySystem = axyraDataRecoverySystem;
});
