/**
 * AXYRA - Sistema de Gestión de Temas y Personalización
 * Maneja temas, colores, fuentes y personalización de la UI
 */
...
class AxyraThemeManager {
  ...
  init() {
    console.log('🎨 Inicializando sistema de gestión de temas...');
    this.loadThemes();
  ...
  loadCustomThemes() {
    try {
      const stored = localStorage.getItem('axyra_custom_themes');
      if (stored) {
        this.customThemes = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando temas personalizados:', error);
    }
  }
  ...
  saveCustomThemes() {
    try {
      localStorage.setItem('axyra_custom_themes', JSON.stringify(this.customThemes));
    } catch (error) {
  ...
  createCustomTheme(themeData) {
    const theme = {
      id: themeData.id || 'custom_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9),
      name: themeData.name,
    ...
    console.log('✅ Tema personalizado creado:', theme.name);
    return theme;
  }
  ...
  updateCustomTheme(themeId, updates) {
    ...
    // Si es el tema actual, aplicarlo
    if (this.currentTheme === themeId) {
      this.applyTheme(this.customThemes[themeIndex]);
    }

    console.log('✅ Tema personalizado actualizado:', this.customThemes[themeIndex].name);
  ...
  deleteCustomTheme(themeId) {
    ...
    // Si era el tema actual, cambiar al tema por defecto
    if (this.currentTheme === themeId) {
      this.setTheme('light');
    }

    console.log('🗑️ Tema personalizado eliminado:', theme.name);
    return theme;
  }
  ...
  createThemeFromPalette(paletteId, baseThemeId = 'light') {
    ...
    console.log('✅ Tema creado desde paleta:', theme.name);
    return theme;
  }
  ...
  exportTheme(themeId) {
    ...
    link.href = url;
    link.download = `axyra-theme-${theme.name.replace(/\s+/g, '-').toLowerCase()}.json`;
  ...
  generateThemePreview(theme) {
    ...
    preview.innerHTML = `
      <div class="preview-header" style="background: ${theme.colors.primary}; color: ${theme.colors.text};">
  ...
  getThemeStatistics() {
    const totalThemes = this.themes.length + this.customThemes.length;
    const customThemes = this.customThemes.length;
    const mostUsedTheme = this.getMostUsedTheme();
  ...
  getMostUsedTheme() {
    // Simular tema más usado (en un entorno real se basaría en estadísticas)
    return this.currentTheme;
  }

  resetToDefaultTheme() {
    this.setTheme('light');
    console.log('🔄 Tema restablecido al por defecto');
  }
    ...
    editor.innerHTML = `
      ...
      <div class="theme-editor-content">
        ...
        <div class="theme-editor-footer">
          <button onclick="axyraThemeManager.saveCustomTheme()" class="btn btn-primary">Guardar Tema</button>
          <button onclick="axyraThemeManager.resetThemeEditor()" class="btn btn-secondary">Resetear</button>
        </div>
      </div>
    `;
  ...
  renderPreviewEditor() {
    ...
    return this.generateThemePreview(currentTheme);
  }
  ...
  updateThemeSpacing(key, value) {
    const currentTheme = this.getCurrentTheme();
    if (currentTheme) {
      currentTheme.spacing[key] = value;
      this.applyTheme(currentTheme);
    }
  }
  ...
  saveCustomTheme() {
    const currentTheme = this.getCurrentTheme();
    if (currentTheme) {
      this.createCustomTheme(currentTheme);
  ...
  resetThemeEditor() {
    this.setTheme('light');
    console.log('🔄 Editor de temas resetado');
  }
}

// Inicializar sistema de gestión de temas
let axyraThemeManager;
document.addEventListener('DOMContentLoaded', () => {
  axyraThemeManager = new AxyraThemeManager();
