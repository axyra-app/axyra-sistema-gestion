/**
 * AXYRA - Sistema de Configuración del Sistema
 * Maneja configuración global, preferencias y ajustes
 */

class AxyraSystemConfiguration {
  constructor() {
    this.config = {
      system: {
  ...
  loadConfiguration() {
    try {
      const stored = localStorage.getItem('axyra_system_config');
      if (stored) {
  ...
  loadPreferences() {
    try {
      ...
      console.warn('Error cargando preferencias del usuario:', error);
    }
  }

  loadSettings() {
    try {
      const stored = localStorage.getItem('axyra_system_settings');
      if (stored) {
        this.settings = JSON.parse(stored);
      }
    } catch (error) {
      console.warn('Error cargando configuraciones del sistema:', error);
    }
  }

  saveConfiguration() {
    try {
      localStorage.setItem('axyra_system_config', JSON.stringify(this.config));
    } catch (error) {
  ...
  savePreferences() {
    try {
      localStorage.setItem('axyra_user_preferences', JSON.stringify(this.preferences));
    } catch (error) {
  ...
  saveSettings() {
    try {
      localStorage.setItem('axyra_system_settings', JSON.stringify(this.settings));
    } catch (error) {
      ...
      reader.onload = (event) => {
        try {
          ...
          if (configData.preferences) {
            this.preferences = { ...this.preferences, ...configData.preferences };
            this.savePreferences();
          }

          if (configData.settings) {
            this.settings = { ...this.settings, ...configData.settings };
            this.saveSettings();
          }
...
let axyraSystemConfiguration;
document.addEventListener('DOMContentLoaded', () => {
  axyraSystemConfiguration = new AxyraSystemConfiguration();
